...
import { _k2LendingContract } from './contracts';
...
export const _getDebtor = async (signer: Signer | Provider, debtor: string) => {
    ...
    const contract = await _k2LendingContract(signer);
...
export const _nodeOperatorWithdraw = async (signer: Signer | Provider, nodeOperator: string, blsPublicKey: string) => {

    const contract = await _k2LendingContract(signer);

    return contract.nodeOperatorWithdraw(_add0x(nodeOperator), _add0x(blsPublicKey));
};

export const _nodeOperatorKick = async (signer: Signer | Provider, reporterAddress: string, blsPublicKey: string) => {

    const contract = await _k2LendingContract(signer);

    return contract.nodeOperatorKick(_add0x(reporterAddress), _add0x(blsPublicKey));
};
...
export const _terminate = async (signer: Signer | Provider) => {
...
export const _setHookAsDebtorForSBP = async (signer: Signer | Provider, hookAddress: string) => {

    const contract = await _k2LendingContract(signer);

    return contract.setHookAsDebtorForSBP(_add0x(hookAddress)) ;
};
